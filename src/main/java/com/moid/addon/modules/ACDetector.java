...
public class ACDetector extends Module {
    ...
    private final Setting<Integer> scanDelay = sgGeneral.add(new IntSetting.Builder()
        .name("scan-delay")
        .description("Tries to find out to find anticheats installed on the server")
        .defaultValue(10)
        .min(1)
        .build()
    );
    ...
    public ACDetector() {
        super(MoidAddon.CATEGORY, "ac-detector", "Strictly identifies Anticheats by intercepting responses.");
    ...
    public void onActivate() {
        ...
        if (!queue.isEmpty()) info(Formatting.GRAY + "Starting scan...");
    }
